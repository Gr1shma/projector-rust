...
impl TryFrom<Vec<String>> for Operation {
    ...
    fn try_from(value: Vec<String>) -> Result<Self> {
        ...
        if term == "add" {
            if value.len() != 3 {
                let args = anyhow!("operation add expect 2 arguments but got {}", value.len()-1);
                return Err(args);
            }
        ...
        if term == "rm" {
            if value.len() != 2 {
                let args = anyhow!("operation remove expect 1 arguments but got {}", value.len()-1);
                return Err(args);
            }
        ...
        if value.len() > 1 {
            let err = anyhow!("operation print expect 0 or 1 arguments but got {}", value.len());
            return Err(err);
        }
        let arg = value.pop().expect("to exist");
